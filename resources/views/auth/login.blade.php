...
@section('content')
    ...
    <div class="p-container">
        ...
        <form action="{{ route('login') }}" method="POST" class="c-form">
            ...
            <input type="password" name="password" placeholder="ぱすわーどを入力" minlength="8" maxlength="16" required class="u-mb__50 @if(!empty($errors->first('password'))) error @endif">
            <!-- エラーテキスト -->
            @if($errors->any())
            ...
            @endif
            <!-- ぱすわーど再設定へ -->
            <a href="{{ route('password.request') }}" class="u-hover u-mb__50">ぱすわーどをお忘れのかたはこちら</a>
            <!-- ボタン -->
